...
class QueueCog(commands.Cog, name="Queue"):
    ...
    async def run_matchmaking_logic(
        ...
        if not game:
            ...
            if ready is True:
                ...
                await ctx.send(
                    f"Um jogador recusou o jogo ou a queue e foi removido do matchmaking\n"
                    f"Todos os players voltaram a queue",
                )
                ...
                await ctx.send(
                    "O tempo de confirmação acabou, os jogadores que não aceitaram foram removidos da queue",
                )
            ...
            await ctx.send(
                f"A melhor partida encontrada havia um dos lados com {(.5 + game.matchmaking_score)*100:.1f}%"
                f" chance prevista de ganhar"
            )
    ...
    async def queue(
        ...
        # If there is a duo, we go for a different flow (which should likely be another function)
            ...
            if not duo_role:
                await ctx.send("Você precisa colocar a lane de seu duo")
                return

            duo_validation_message = await ctx.send(
                f"<@{ctx.author.id}> {get_role_emoji(role)} quer jogar junto com <@{duo.id}> {get_role_emoji(duo_role)}\n"
                f"Pressione o ✅ para aceitar o duo"
            )
            ...
            if not validated:
                await ctx.send(f"<@{ctx.author.id}>: Duo recusado!")
                return
    ...
    async def won(
        ...
        with session_scope() as session:
            ...
            if not game:
                await ctx.send("Você nunca jogou um jogo neste server")
                return

            elif game and game.winner:
                await ctx.send(
                    "Seu ultimo jogo ja foi marcado como vencedor\n"
                    "Se houve algum problema, por favor chame um dos mods/ajudantes do Yoinhouse!"
                )
                return
            ...
            win_validation_message = await ctx.send(
                f"{game.players_ping}"
                f"{ctx.author.display_name} quer marcar o jogo {game.id} como vitória para o lado {participant.side}\n"
                f"O resultado irá ser validado após 7 confirmações clicando no ✅"
            )
            ...
            if not validated:
                await ctx.send("A confirmação de resultado foi cancelada, tente novamente")
                return

            # If we get there, the score was validated and we can simply update the game and the ratings
            queue_channel_handler.mark_queue_related_message(
                await ctx.send(
                    f"O jogo {game.id} foi marcado como vitória para o lado {participant.side} e os rankings foram atualizados"
                )
            )
    ...
    @doc(f"""
        ...
        Will require validation from at least 7 players in the game
    ...
    async def cancel(
        ...
        with session_scope() as session:
            ...
            if game and game.winner:
                await ctx.send("Aparentemente você não faz parte de algum jogo em andamento")
                return

            elif game.id in self.games_getting_scored_ids:
                await ctx.send("Já existe uma mensagem de resultado ou cancelamento ativa para este jogo")
                return
            ...
            cancel_validation_message = await ctx.send(
                f"{game.players_ping}"
                f"{ctx.author.display_name} quer cancelar o jogo {game.id}\n"
                f"Jogo será cancelado uma vez que 7 jogadores apertarem ✅"
            )
            ...
            if not validated:
                ...
                queue_channel_handler.mark_queue_related_message(
                    await ctx.send(f"Jogo {game.id} foi cancelado")
                )
