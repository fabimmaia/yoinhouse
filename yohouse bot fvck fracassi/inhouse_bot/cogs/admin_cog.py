...
class AdminCog(commands.Cog, name="Admin"):
    ...
    @doc(f"Funções de admin, use {PREFIX}help admin para a lista completa")
    async def admin(self, ctx: commands.Context):
        if ctx.invoked_subcommand is None:
            await ctx.send(
                f"os subcomandos aceitos são"
    ...
    async def reset(
        ...
        if not member_or_channel or type(member_or_channel) == discord.TextChannel:
            ...
            await ctx.send(f"A queue foi resetada no canal {channel.name}")

        elif type(member_or_channel) == discord.Member:
            game_queue.remove_player(member_or_channel.id)
            await ctx.send(f"{member_or_channel.name} todos os jogadores foram removidos de suas respectivas queues")
    ...
    async def won(self, ctx: commands.Context, member: discord.Member):
        ...
        await ctx.send(
            f"O resultado do jogo foi alterado para o time de {member.display_name}"
            f"Os rankings foram atualizados"
        )
    ...
    async def cancel(self, ctx: commands.Context, member: discord.Member):
        ...
        with session_scope() as session:
            ...
            if game and game.winner:
                await ctx.send("O jogo ja obeteve um resultado, ele não poderá mais ser cancelado.")
                return

            session.delete(game)

        await ctx.send(f"{member.display_name} seu jogo foi cancelado e removido da database")
        await queue_channel_handler.update_queue_channels(bot=self.bot, server_id=ctx.guild.id)
    ...
    async def mark(self, ctx: commands.Context, channel_type: str):
        ...
        if channel_type.upper() == "QUEUE":
            ...
            await ctx.send(f"Este canal foi marcado como um canal de queue!")
            ...
            await ctx.send(f"Este canal foi marcado como um canal de ranking!")
    ...
    async def unmark(self, ctx: commands.Context):
        ...
        await ctx.send(f"O canal voltou a ser um canal comum!")
